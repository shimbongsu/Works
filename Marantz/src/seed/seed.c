...
/*
static unsigned char gUserKey[SEED_USER_KEY_LEN] = {
	'K', 'E', 'Y', '-', 'L', 'G', '-', 'N', 
	'S', 'Y', 'S', '-', 'A', 'J', 'M', 'S'
};
static unsigned char gIV[SEED_BLOCK_LEN] = {
	'I', 'V', '-', '-', 'L', 'G', '-', 'N', 
	'S', 'Y', 'S', '-', 'A', 'J', 'M', 'S'
};
*/

static unsigned char gUserKey[SEED_USER_KEY_LEN] = {"1234567890ABCDEF"};
static unsigned char gIV[SEED_USER_KEY_LEN]      = {"1234567890ABCDEF"};

int seed_cfb_fixed_enc(unsigned char *srcData, unsigned int srcLen, unsigned char *dstData, unsigned int *dstLen)
{
	RET_VAL	ret;  
	SEED_ALG_INFO algInfo;
	unsigned int _dstLen;
	unsigned char *_dstBuf;
	unsigned char *copyp;

	if(srcData == 0 || srcLen == 0 || dstData == 0 || dstLen == 0) {
		goto FAIL;
	}
	
	_dstBuf = (unsigned char *)calloc(1, srcLen);

	SEED_SetAlgInfo(AI_ECB, AI_NO_PADDING, gIV, &algInfo);
	//SEED_SetAlgInfo(AI_CFB, AI_PKCS_PADDING, gIV, &algInfo);
	ret = SEED_KeySchedule(gUserKey, (unsigned int)SEED_USER_KEY_LEN, &algInfo);
	if(ret!=CTR_SUCCESS) {
		goto FAIL;
	}
	...
	//_dstLen = srcLen;
	_dstLen = dstLen;
	ret = SEED_EncUpdate(&algInfo, srcData, srcLen, _dstBuf, &_dstLen);
	if(ret!=CTR_SUCCESS) {
		goto FAIL;
	}
	
	copyp = dstData;
	memcpy(copyp, _dstBuf, _dstLen);
	*dstLen = _dstLen;
	copyp = copyp + _dstLen;
	
	_dstLen = srcLen - _dstLen;
	ret = SEED_EncFinal(&algInfo, _dstBuf, &_dstLen);
	if(ret!=CTR_SUCCESS) {
		goto FAIL;
	}
...
FAIL :
	if(dstData != 0) memset(dstData, 0x00, *dstLen);
	if(dstLen) *dstLen = 0;
	if(_dstBuf) free(_dstBuf);
	
	log_printf(ERR_ARG,"SEED RETURN : [0x%04x]",ret);
	
	return 0;
	
}

int seed_cfb_fixed_dec(unsigned char *srcData, unsigned int srcLen, unsigned char *dstData, unsigned int *dstLen)
{
	RET_VAL	ret;
	SEED_ALG_INFO algInfo;
	unsigned int _dstLen;
	...
	if(srcData == 0 || srcLen == 0 || dstData == 0 || dstLen == 0) goto FAIL;
	...
	//_dstLen = srcLen;
	_dstLen = dstLen;
	ret = SEED_DecUpdate(&algInfo, srcData, srcLen, _dstBuf, &_dstLen);
	if(ret!=CTR_SUCCESS) {
		goto FAIL;
	}
	...
	*dstLen = _dstLen;
	...
	_dstLen = srcLen - _dstLen;
	ret = SEED_DecFinal(&algInfo, _dstBuf, &_dstLen);
	if(ret!=CTR_SUCCESS) {
		goto FAIL;
	}
