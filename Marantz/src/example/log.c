/***********************************************************
 SRC Name		: log.c 
 INC Header		: log.h
 Function		: Business Utility Function
...
int GetLocalTime(char *tmBuff, char *Type, int tmBuffSize)
{
	...
	if( strncmp(Type,"S",1) == 0 ){
		strftime(lc_tmbuff,40,"%H:%M:%S",tmp); /* YYYY-MM-DD HH:MM:SS */
	}
				...
				const char *fmsg, ...)
{
	...
	*/
	...
	if ( LogLevel == 0 ) {
/*		sprintf(format,"[LOG][%s]:[%s][%s()\t][%d]::[%s]\n",lc_DateSec,FileName,FunctionName,LineNumber,fmsg);*/
		sprintf(format,"[LOG][%s]:[%12s][%04d]::%s\n",lc_DateSec,FileName,LineNumber,fmsg);
	} else if ( LogLevel == 1 ) {
/*		sprintf(format,"[ERR][%s]:[%s][%s()\t][%d]::[%s]\n",lc_DateSec,FileName,FunctionName,LineNumber,fmsg);*/
		sprintf(format,"[ERR][%s]:[%12s][%04d]::%s\n",lc_DateSec,FileName,LineNumber,fmsg);
	} else if ( LogLevel == 2 ) {
/*		sprintf(format,"[DBG][%s]:[%s][%s()\t][%d]::[%s]\n",lc_DateSec,FileName,FunctionName,LineNumber,fmsg);*/
...
LOG_CREATE:
	...
	if(file == -1 || fdate != cdate)
	{
		...
		sprintf(path, "%s/work/log/test_%s", getenv("HOME"), lc_Date);
...
int rTrim(char *ToStr)
{
    ...
    memset(lc_Buff,  0x00, sizeof(lc_Buff));
    strcpy(lc_Buff,  ToStr);
    memset(ToStr, 0x00, strlen(ToStr));
    for(i=strlen(lc_Buff)-1; i>=0; i--) {
       if(lc_Buff[i] != ' ') {
          memset(ToStr, 0x00, strlen(ToStr));
          strncpy(ToStr, lc_Buff, i+1);
          break;
       }
    }
    return RETURN_TRUE;/* return TRUE */
